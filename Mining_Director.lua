...
local function main()
    local x=1
    local z=1
    if fileExists('disk/commands.txt')==true then
        
        if type(readFile('disk/commands.txt')[1][1])~="nil"then
            x=-readFile('disk/commands.txt')[1][1]
            z=readFile('disk/commands.txt')[1][3]
        end

    end
    while true do
        for x = x, x+16*4, 16 do
            print("sending to:",-x,0,z)
            writeFile('disk/commands.txt',{{-x,0,z}})
            print('wait ON')
