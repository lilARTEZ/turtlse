...
local function mineChunk()
    ...
    RunProtected(goTo,{chunkStart[1],chunkStart[2],chunkStart[3]})
    ...
    while true do
        for z = 1, 16, 1 do
            for x = 1, 16, 1 do
                if z%2==0 then
                    RunProtected(goTo,{chunkStart[1]+17-x,chunkStart[2]+y,chunkStart[3]+z})
                else 
                    RunProtected(goTo,{chunkStart[1]+x,chunkStart[2]+y,chunkStart[3]+z})
                end
            end
        end
        y=y-1
        if inventoryFulness()==true then
            dump()
        end
        if  y==-315 then
            break
...
local function main()
    ...
    turtle.suck(1)
    turtle.select(2)
    turtle.suckUp(1)--slot 1 dump  slot 2 fuel
    turtle.turnLeft()
    refuel()
    goTo({0,0,1})
    mineChunk()
    RunProtected(goTo,{0,0,-1})
end
